<template>
  <div class="container">
    <el-card>
      <el-row>
        <el-button type="primary" plain size="medium" @click="showAddStockIn"
          >新增库存盘点</el-button
        >
      </el-row>
      <el-table
        :data="stockInTableData"
        border
        stripe
        hover
        style="width: 100%"
      >
        <el-table-column prop="id" fixed label="ID" width="150">
        </el-table-column>

        <el-table-column prop="createDate" fixed label="创建时间" width="150">
        </el-table-column>

        <el-table-column prop="code" label="单据编号" width="150">
        </el-table-column>

        <el-table-column prop="makerName" label="制单人" width="150">
        </el-table-column>

        <el-table-column prop="orderDate" label="制单时间" width="150">
        </el-table-column>

        <el-table-column prop="storeId" label="仓库编号" width="150">
        </el-table-column>

        <el-table-column prop="updateDate" label="更新时间" width="150">
        </el-table-column>

        <el-table-column label="单据状态" width="150">
          <template slot-scope="scope">
            <div>
              <el-button type="danger" plain v-if="scope.row.status === 1"
                >未审核</el-button
              >
              <el-button type="primary" plain v-else>审核</el-button>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="审核" width="150">
          <template slot-scope="scope">
            <div>
              <el-button
                type="success"
                size="medium"
                plain
                @click="checkOrder(scope.row.id)"
                >审核</el-button
              >
            </div>
          </template>
        </el-table-column>
        <el-table-column label="反审核" width="150">
          <template slot-scope="scope">
            <el-button
              type="primary"
              plain
              size="small"
              @click="uncheckOrder(scope.row.id)"
              >反审核</el-button
            >
          </template>
        </el-table-column>
        <el-table-column fixed="right" label="操作" width="100">
          <template slot-scope="scope">
            <el-button
              type="warning"
              icon="el-icon-edit"
              size="mini"
              circle
              @click="editOrderVisible(scope.row.id)"
            ></el-button>
            <el-button
              type="danger"
              icon="el-icon-delete"
              size="mini"
              circle
              @click="removeStockIn(scope.row.id)"
            ></el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
      <pagination
        :parentCurrentPage="pages.page"
        :parentPageSizes="pages.pages"
        :parentPageSize="pages.rows"
        :parentTotal="pages.count"
        @parentGetData="getStockChecked"
      ></pagination>
      <!-- 添加 -->
      <el-dialog
        title="添加入库单"
        :visible.sync="stockInDialogVisible"
        width="50%"
        @close="cancelStockIn"
      >
        <span>
          <el-form
            :model="stockInForm"
            :rules="stockInrules"
            ref="stockInRef"
            label-width="100px"
          >
            <el-form-item label="入库单号" prop="code">
              <el-input v-model="stockInForm.code"></el-input>
            </el-form-item>
            <el-form-item label="仓库ID" prop="storeId">
              <el-select v-model="stockInForm.storeId">
                <el-option label="上海仓库" value="1"></el-option>
                <el-option label="北京仓库" value="2"></el-option>
                <el-option label="华北仓库" value="3"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="制单人ID" prop="makerName">
              <el-input v-model="makerName" disabled></el-input>
            </el-form-item>
            <el-form-item label="制单时间" prop="orderDate">
              <el-date-picker
                type="date"
                value-format="yyyy-MM-dd"
                placeholder="选择日期"
                v-model="stockInForm.orderDate"
                style="width: 100%"
              ></el-date-picker>
            </el-form-item>
          </el-form>
        </span>
        <span slot="footer" class="dialog-footer">
          <el-button @click="cancelStockIn">取 消</el-button>
          <el-button type="primary" @click="addStockIn">确 定</el-button>
        </span>
      </el-dialog>
      <!-- 
        编辑
       -->
      <el-drawer
        title="编辑盘点单"
        :visible.sync="stockInVisible"
        :direction="direction"
        :before-close="handleClose"
      >
        <span>
          <el-form
            :model="editOrderList"
            :rules="editOrderListRules"
            ref="editOrderListRef"
            label-width="100px"
          >
            <el-form-item label="订单ID" prop="id">
              <el-input v-model="editOrderList.id" disabled></el-input>
            </el-form-item>

            <el-form-item label="订单编号" prop="code">
              <el-input v-model="editOrderList.code"></el-input>
            </el-form-item>

            <el-form-item label="制单人ID" prop="makerId">
              <el-input v-model="editOrderList.makerId"></el-input>
            </el-form-item>

            <el-form-item label="仓库编号" prop="storeId">
              <el-select v-model="editOrderList.storeId">
                <el-option label="上海仓库" value="1"></el-option>
                <el-option label="北京仓库" value="2"></el-option>
                <el-option label="华北仓库" value="3"></el-option>
              </el-select>
            </el-form-item>

            <el-form-item label="制单时间" prop="orderDate">
              <el-date-picker
                type="date"
                value-format="yyyy-MM-dd"
                placeholder="选择日期"
                v-model="editOrderList.orderDate"
                style="width: 100%"
              ></el-date-picker>
            </el-form-item>
          </el-form>
          <el-button type="primary" @click="editStockIn">确定</el-button>
        </span>
      </el-drawer>
    </el-card>
  </div>
</template>

<script>
import Pagination from "../../components/Pagination.vue";

import qs from "qs";
export default {
  data() {
    return {
      stockInTableData: [],
      stockInDialogVisible: false,
      stockInVisible: false,
      direction: "rtl",
      formInLine: {
        code: "",
        time: [],
        maker: "",
        status: "",
        storeId: "",
      },
      pages: {
        count: 0, //总记录的条数
        page: parseInt(window.sessionStorage.getItem("newSize")) || 1, //第几页
        rows: parseInt(window.sessionStorage.getItem("newSize")) || 5, //显示条数
        pages: [5, 10, 15, 20],
      },
      stockInForm: {
        code: "",
        storeId: "",
        makerId: window.sessionStorage.getItem("userId"),
        orderDate: "",
      },
      makerName: window.sessionStorage.getItem("userName"),
      stockInrules: {
        code: [
          { required: true, message: "请输入入库单号", trigger: "blur" },
          {
            min: 8,
            max: 15,
            message: "长度在 8 到 15 个字符",
            trigger: "blur",
          },
        ],
        stocskId: [{ required: true, message: "请选择仓库", trigger: "blur" }],
        makerId: [
          { required: true, message: "请输入制单人ID", trigger: "blur" },
        ],
        orderDate: [
          { required: true, message: "请输入仓库ID", trigger: "blur" },
          {
            min: 3,
            max: 15,
            message: "长度在 3 到 15 个字符",
            trigger: "blur",
          },
        ],
      },
      editOrderList: {
        code: "",
        id: 0,
        makerId: 1,
        orderDate: "",
        storeId: "",
      },
      editOrderListRules: {
        code: [
          { required: true, message: "请输入入库单号", trigger: "blur" },
          {
            min: 1,
            max: 15,
            message: "长度在 1 到 15 个字符",
            trigger: "blur",
          },
        ],
        stocskId: [
          { required: true, message: "请选择仓库", trigger: "blur" },
          {
            min: 1,
            max: 15,
            message: "长度在 1 到 15 个字符",
            trigger: "blur",
          },
        ],
        makerId: [
          { required: true, message: "请输入制单人ID", trigger: "blur" },
          {
            min: 1,
            max: 15,
            message: "长度在 1 到 15 个字符",
            trigger: "blur",
          },
        ],
        orderDate: [
          { required: true, message: "请输入仓库ID", trigger: "blur" },
          {
            min: 3,
            max: 15,
            message: "长度在 3 到 15 个字符",
            trigger: "blur",
          },
        ],
      },
    };
  },
  methods: {
    async getStockChecked(page, rows) {
      console.log(this.formInLine);
      console.log(this.pages);
      //  设置入库请求参数
      let { code, time, maker, status, storeId } = this.formInLine;
      let params = {
        code,
        startDate: time ? time[0] : "",
        endDate: time ? time[1] : "",
        maker,
        status,
        storeId,
        page: this.pages.page,
        rows: this.pages.rows,
      };
      /*  params.page = page || this.pages.page;
      params.rows = rows || this.pages.rows; */

      // 当前页
      params.page =
        page ||
        parseInt(window.sessionStorage.getItem("newPage")) ||
        this.pages.page;
      // 一共第几页
      params.rows =
        rows ||
        parseInt(window.sessionStorage.getItem("newSize")) ||
        this.pages.rows;
      console.log(params.page);
      console.log(params.rows);

      // console.log(params);
      params = qs.stringify(params);
      // console.log(params);
      const { data: res } = await this.$http.get("stockcheck/list?" + params);
      console.log(res.data.data);
      // console.log(res.data);
      this.stockInTableData = res.data.data;

      this.pages.count = res.data.page.count;
    },
    /* 
    审核
     */
    async checkOrder(id) {
      let params = {
        id,
      };
      params = qs.stringify(params);
      const { data: res } = await this.$http.put("stockcheck/check?" + params);
      if (res.code === 1) {
        this.$message({
          message: "审核成功",
          type: "success",
        });
        this.getStockChecked();
      } else {
        this.$message({
          message: "审核失败",
          type: "error",
        });
      }
    },
    /* 
    反审核
     */
    async uncheckOrder(id) {
      let params = { id };
      params = qs.stringify(params);
      const { data: res } = await this.$http.put(
        "stockcheck/uncheck?" + params
      );
      if (res.code === 1) {
        this.$message({
          message: "反审核成功",
          type: "success",
        });
        this.getStockChecked();
      } else {
        this.$message({
          message: "反审核失败",
          type: "error",
        });
      }
    },

    /* 
    新增库存盘点
    */
    showAddStockIn() {
      this.stockInDialogVisible = true;
    },
    cancelStockIn() {
      //  清空表单
      this.$refs.stockInRef.resetFields();
      this.stockInDialogVisible = false;
    },
    addStockIn() {
      this.$refs.stockInRef.validate(async (valid) => {
        if (!valid) {
          this.$message.error("添加失败");
          return false;
        }
        let stockInForm = qs.stringify(this.stockInForm);
        console.log(stockInForm);
        const { data: res } = await this.$http.post("stockcheck", stockInForm);
        console.log(res);
        if (res.code === 1) {
          this.$message({
            message: "添加成功",
            type: "success",
          });
          this.stockInDialogVisible = false;
          this.getStockChecked();
        } else {
          this.$message({
            message: "添加失败",
            terrorype: "error",
          });
        }
      });
    },
    /* 
    删除
    */
    async removeStockIn(id) {
      const confirmResult = await this.$confirm(
        "此操作将永久删除该入盘点单, 是否继续?",
        "提示",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        }
      ).catch((err) => err);
      console.log(confirmResult);
      // 如果确认删除返回confirm字符串，点击取消返回cancel
      // 会触发$confirm中的错误，使用catch捕获错误
      if (confirmResult !== "confirm") {
        return this.$message("取消删除");
      }
      let params = { id };
      params = qs.stringify(params);
      const res = await this.$http.delete("stockcheck?" + params);
      if (res.data.code === 1) {
        this.$message({
          message: "删除入库单成功",
          type: "success",
        });
        this.getStockChecked();
      } else {
        this.$message({
          message: "删除入库单失败",
          type: "error",
        });
      }
    },
    /* 
    分页
    */
    /* handleSizeChange(newSize) {
      this.pages.rows = newSize;
      this.getStockChecked();
    }, */
    handleSizeChange(newSize) {
      this.pageSize = newSize;
      window.sessionStorage.setItem("newSize", newSize);
      if (Math.ceil(this.parentTotal / newSize) > this.currentPage) {
        this.$emit("parentGetData", this.currentPage, this.pageSize);
      } else {
        this.$emit("parentGetData", 1, this.pageSize);
      }
      this.getStockChecked();
    },
    handleCurrentChange(newPage) {
      this.pages.page = newPage;
      this.getStockChecked();
    },
    /* 
    编辑
    */
    editOrderVisible(id) {
      this.editOrderList.id = id;
      this.stockInVisible = true;
    },
    handleClose(done) {
      this.$confirm("确认关闭？")
        .then((_) => {
          done();
        })
        .catch((_) => {});
      this.$refs.editOrderListRef.resetFields();
    },

    async editStockIn() {
      let params = this.editOrderList;
      params = qs.stringify(params);
      const { data: res } = await this.$http.post("stockcheck/update", params);
      console.log(res);
      if (res.code === 1) {
        this.$message({
          message: "修改订单成功",
          type: "success",
        });
        this.stockInVisible = false;
        this.getStockChecked();
      } else {
        this.$message({
          message: "修改订单失败",
          type: "error",
        });
      }
    },
  },

  components: {
    Pagination,
  },
  created() {
    this.getStockChecked();
  },
  mounted() {},
};
</script>
<style>
</style>
